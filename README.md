...
###### Докер использует автогенерацию имен контейнеров, что может приводить к не консистентному поведению команд со строго установленными именами, чтобы этого избежать, мы используем доступ по имени сервиса прописанного в нашем docker-compose файле (*"app" для миграций и "db" для базы данных*) 
```bash
docker compose exec app sh -c "poetry run alembic revision --autogenerate -m 'initial tables'"
```

#### 4. Применяем миграцию
```bash
docker compose exec app sh -c "poetry run alembic upgrade head"
```
...
Можно использовать готовый скрипт загрузки тестовых данных *init_data* 

(**ВАЖНО!**: Он очищает существующие таблицы!)
```
docker compose exec app python -m app.init_data
```


Или произвести ввод подключившись к контейнеру
**Пример**:
```bash
docker compose exec db psql -U postgres -d product_categories
```
...
- Преобразовал скрипт добавления тестовых данных
- Изменил команды для развертывания приложения на более универсальные
- Добавлены pytest в зависимости и unit-тесты для проверки работы функции "добавления товара"